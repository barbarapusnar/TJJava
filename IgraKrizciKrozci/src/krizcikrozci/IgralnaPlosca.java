...
return new int[]{xPovr+st*sPolje,yPovr+vr*sPolje};
    ...
    private void narišiKrižec(Graphics g, int i, int j) {
        ...
        g.drawLine(xy[0]+sPolje/8,xy[1]+sPolje/8,
                xy[0]+7*sPolje/8,xy[1]+7*sPolje/8);
        g.drawLine(xy[0]+sPolje/8,xy[1]+7*sPolje/8,
                xy[0]+7*sPolje/8,xy[1]+sPolje/8);
    }
    ...
    private int preveriZmago(int vr, int st) {
       // throw new UnsupportedOperationException("Not yet implemented");
        int število=0;
        for(int j=0;j<3;j++)
            if(polja[vr][j]==naPotezi) število++;
        if (število==3) return vr; //tris v vr vrstici
        število=0;
        for(int j=0;j<3;j++)
            if(polja[j][st]==naPotezi) število++;
        if (število==3) return st+3; //tris v stolpcu 3,4,5
       // /glavna diagonala
        število=0;
        if(vr==st)
        {
        for(int i=0;i<3;i++)
            if(polja[i][i]==naPotezi) število++;
        if (število==3) return 6;
        } //tris je v diagonali 11 22 33
        število=0;
        if(vr+st==2)
        {
        for(int i=0;i<3;i++)
            if(polja[2-i][i]==naPotezi) število++;
        if (število==3) return 7; //tris je v diagonali 31 22 13
        }
        return -1;
    }
}
